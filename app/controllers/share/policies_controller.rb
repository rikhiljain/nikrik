class Share::PoliciesController < ApplicationController
  ...
  def index
    @motor_policies = Share::Policy.all
  ...
  def show
    @motor_policy = Share::Policy.find(params[:id])
  ...
  def edit
    ...
    @motor_policies = Share::Policy.where("user_id=?", @motor_policy.user_id)
    @is_edit = true
  ...
  def create
    @motor_policy = Share::Policy.new(params[:motor_policy])
     @is_edit = false
    upload
  ...
  def update
    @motor_policy = Share::Policy.find(params[:id])
    new_motor_policy = Share::Policy.new(params[:motor_policy])
    ...
    respond_to do |format|
      if @motor_policy.update_attributes(Share::Policy.to_hash(new_motor_policy))
        format.html { redirect_to "/admin_users/#{@motor_policy.user_id}" , notice: 'Motor policy was successfully created.' }
