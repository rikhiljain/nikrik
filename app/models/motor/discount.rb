class Motor::Discount < ActiveRecord::Base
  self.table_name = "motor_discounts"

  attr_accessible :amount, :company_id, :idv_chart_id, :rto_id, :year
  belongs_to :company,  :class_name => 'Company'

  belongs_to :idv_chart
  belongs_to :rto, :class_name => 'Motor::Rto' 

  validates :amount, :company_id, :idv_chart_id, :presence => true
  validates :amount, :numericality => true

  def self.get_discount(idv_chart_id,company_id,rto_id, year)
    where("idv_chart_id = ? and company_id = ? and rto_id in (0,?) and year = ?",idv_chart_id,company_id,rto_id, year).order("rto_id desc").first
  end

  def self.find_by_company(company_id)
    where("company_id = ?",company_id).order("rto_id desc")
  end

end
