...
var TravelQuoteResult = (function($){
	...
	that.fillResults = function(data){
		...
		var e = esc;
		var html = [], h = -1;
		html[++h] = "<p><button id='travelQuoteResultsRecomputeLink' class='btn is_btn-custom' type='button' onclick='TravelQuoteResult.recomputeQuote()'>Recompute</button>";
		html[++h] = "&nbsp;&nbsp;"; 
	...
	that.getPremiumBreakUp = function(options){
		...
		var html = [], h = -1;
		if(!modal){
			html[++h] = "<h3>Travel Insurance Premium for - " + lastClickedQuote.company_name + "</h3>";
			html[++h] = "<div>";
		}
